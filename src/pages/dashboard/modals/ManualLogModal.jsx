...
export function ManualLogModal({ token, inventory, onSave, onClose }) {
  const [label,    setLabel]    = useState('')
  const [type,     setType]     = useState('fertilise')
  const [zones,    setZones]    = useState([])
  const [products, setProducts] = useState([{ name:'', amount:'', unit:'mL' }])
  ...
  function removeProduct(i) { setProducts(ps => ps.filter((_,j) => j!==i)) }
  ...
  const prodNames = (inventory?.products || []).map(p => p.name)
          ...
          <div className={s.form}>
            ...
            <div>
              ...
              <select className={s.input} style={{ width:'100%', appearance:'none' }} value={type} onChange={e => setType(e.target.value)}>
                {TASK_TYPES.map(t => <option key={t} value={t}>{t==='soilwetter'?'Soil Wetter':t.charAt(0).toUpperCase()+t.slice(1)}</option>)}
              </select>
            </div>
            <div>
            ...
            <div>
              ...
              {products.map((p, i) => (
                <div key={i} style={{ display:'flex', gap:6, marginBottom:8, alignItems:'center' }}>
                  ...
                  <select className={s.input} style={{ flex:'0 0 60px', appearance:'none' }} value={p.unit} onChange={e => updateProduct(i,'unit',e.target.value)}>
