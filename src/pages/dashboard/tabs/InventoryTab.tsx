...
export function InventoryTab({ state, onOpenStock, onOpenResupply }: InventoryTabProps) {
  ...
  const categories = useMemo(() => {
    const cats = ['All']
    products.forEach((p: any) => { if (!cats.includes(p.category)) cats.push(p.category) })
    return cats
  }, [products])
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(240px,1fr))', gap: 12 }}>
        {filtered.map((p: any) => {
          ...
          if (!s) return null
          const CAT_COLORS: Record<string, string> = { Fertiliser: '#4a7c3f', Biostimulant: '#7b5ea7', 'Soil Wetter': '#3a7a8a', Fungicide: '#c0882a', Herbicide: '#c0692a', Insecticide: '#2a7aa0' }
          ...
          return (
            <div key={p.name} onClick={() => onOpenStock(p)}
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 10, fontWeight: 500, flexShrink: 0, alignSelf: 'flex-start', ...badgeStyle[s.status as string] }}>
