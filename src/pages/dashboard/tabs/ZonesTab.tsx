...
export function ZonesTab({ state }: ZonesTabProps) {
  ...
  function tasksThisMonth(zoneId: string) {
    if (!monthData) return 0
    return monthData.weeks.reduce((s: number, w: any) =>
      s + (w.tasks || []).filter((t: any) => (t.zones || []).includes(zoneId)).length
    , 0)
  }
  ...
  return (
    <div>
      ...
      <div className={db.panel}>
        ...
        {(inventory?.compatibility || []).map((rule: any, i: number) => (
