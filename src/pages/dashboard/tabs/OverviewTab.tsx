...
export function OverviewTab({ state, onLogObservation, onDeleteWeather, onNavigateTab }: OverviewTabProps) {
  ...
  const remaining = useMemo(() =>
    allTasksForMonth(program, mn).filter(({ task }: any) => !completions[task.id])
    ...
    for (let offset = 0; offset <= 1; offset++) {
      ...
      for (const weekNum of [1, 3]) {
        const weekTasks = monthTasks
          .filter(({ week }: any) => week === weekNum)
          .filter(({ task }: any) => !completions[task.id])
        if (weekTasks.length > 0) {
          return {
            monthNum: targetMn,
            monthName: MONTHS[targetMn - 1],
            week: weekNum,
            tasks: weekTasks.map(({ task }: any) => task),
  ...
  const alertedProducts = useMemo(() =>
    (inventory?.products || []).filter((p: any) => {
    ...
    <div>
      ...
      {nextUp && (
        <div className={db.panel} style={{ borderLeft: '3px solid var(--grass)' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
            {nextUp.tasks.slice(0, 5).map((task: any) => (
              <span key={task.id} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, padding: '3px 9px', borderRadius: 4, background: 'var(--cream)', border: '1px solid var(--border-light,#ede8de)' }}>
                <span style={{ width: 7, height: 7, borderRadius: '50%', background: (TYPE_COLORS as any)[task.taskType] || '#888', display: 'inline-block', flexShrink: 0 }} />
                {task.label}
      ...
      <div className={db.panel}>
        ...
        {remaining.length === 0
          ...
          : <>
              {remaining.slice(0, 8).map(({ task, week }: any) => (
                <div key={task.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'var(--paper)', border: '1px solid var(--border-light,#ede8de)', borderRadius: 5, fontSize: 14, marginBottom: 8 }}>
                  <span style={{ width: 9, height: 9, borderRadius: '50%', background: (TYPE_COLORS as any)[task.taskType] || '#888', flexShrink: 0, display: 'inline-block' }} />
