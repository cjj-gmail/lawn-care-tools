...
export function ProgramTab({ state }) {
  ...
  const allTasks = useMemo(() => {
    if (!program) return []
    return program.months.flatMap(m =>
      m.weeks.flatMap(w =>
        (w.tasks || []).map(t => ({ task: t, month: m, week: w.week }))
      )
    )
  }, [program])

  const filtered = useMemo(() =>
    filter === 'all' ? allTasks : allTasks.filter(({ task }) => task.taskType === filter)
  , [allTasks, filter])

  const filters = ['all', ...TASK_TYPES]
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:28 }}>
        {program?.months?.map(m => {
          ...
          const SEASON_COLORS = { summer:'#d4a017', autumn:'#c07030', winter:'#4a80b0', spring:'#5a9448' }
          return (
            <Link
              key={m.monthNum}
      ...
      <div className={db.panel}>
        ...
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
            ...
            <tbody>
              {filtered.map(({ task, month, week }) => {
                const isDone = !!completions[task.id]
                const color  = TYPE_COLORS[task.taskType] || '#888'
                return (
