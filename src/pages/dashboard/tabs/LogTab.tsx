...
export function LogTab({ state, onOpenManualLog }: LogTabProps) {
  ...
  const taskTypes = useMemo(() => {
    const t = ['All']
    entries.forEach((e: any) => { if (e.taskType && !t.includes(e.taskType)) t.push(e.taskType) })
    return t
  }, [entries])

  const filteredLog = logFilter === 'All' ? entries : entries.filter((e: any) => e.taskType === logFilter)
    ...
    Object.fromEntries(ZONE_ORDER.map(zid => {
      ...
      if (count >= 2) {
        ...
        for (let i = 1; i < zw.length; i++) {
          const d = Math.round((new Date(zw[i].dateISO).getTime() - new Date(zw[i - 1].dateISO).getTime()) / 86400000)
          if (d > 0) iv.push(d)
        }
    ...
    Object.fromEntries(ZONE_ORDER.map(zid => {
      ...
      if (count >= 2) {
        ...
        for (let i = 1; i < zm.length; i++) {
          const d = Math.round((new Date(zm[i].dateISO).getTime() - new Date(zm[i - 1].dateISO).getTime()) / 86400000)
          if (d > 0) iv.push(d)
        }
        if (iv.length) avg = Math.round(iv.reduce((a, b) => a + b, 0) / iv.length)
      }
  ...
  function exportCSV() {
    ...
    const rows = [headers, ...entries.map((e: any) => [
      e.date || '', e.taskLabel || e.taskType || '', e.taskType || '',
      (e.zones || []).map((z: string) => (ZONES as any)[z]?.name || z).join('; '),
      (e.products || []).map((p: any) => p.name).join('; '),
      (e.products || []).map((p: any) => p.amount || '').join('; '),
      (e.products || []).map((p: any) => p.unit || '').join('; '),
      e.inventoryDeducted ? 'Yes' : 'No', e.manual ? 'Yes' : 'No', e.notes || '',
    ])]
    const csv = rows.map(r => r.map((c: any) => { const s = String(c); return (s.includes(',') || s.includes('"') || s.includes('\n')) ? '"' + s.replace(/"/g, '""') + '"' : s }).join(',')).join('\n')
  ...
  const cellStyle: React.CSSProperties = { padding: '9px 12px', borderBottom: '1px solid var(--border-light,#ede8de)', verticalAlign: 'middle' }
    ...
    <div>
      ...
      <div className={db.statGrid}>
        {[
          ...
          { val: entries.filter((e: any) => e.inventoryDeducted).length, label: 'Inventory deducted', sub: 'of ' + entries.length + ' entries' },
      ...
      <div className={db.panel}>
          ...
          {ZONE_ORDER.map(zid => {
            ...
            return (
              <div key={zid} style={{ background: overdue ? '#fff9e8' : last ? 'white' : 'var(--paper)', border: '1px solid ' + (overdue ? '#d4a017' : 'var(--border)'), borderRadius: 6, padding: '12px 14px' }}>
                <div style={{ fontWeight: 500, fontSize: 13 }}>{(ZONES as any)[zid]?.name}</div>
                <div style={{ fontSize: 12, color: 'var(--ink-light)', marginBottom: 6 }}>{(ZONES as any)[zid]?.grass}</div>
                {last
                  ? <><div style={{ fontSize: 20, fontFamily: 'var(--font-serif)', color: overdue ? '#8a5a00' : 'var(--grass)', fontWeight: 300 }}>{daysSince(last.dateISO)}d ago</div><div style={{ fontSize: 11, color: 'var(--ink-light)' }}>{last.date} &middot; {(WATER_METHOD_LABELS as any)[last.method] || last.method || ''}{last.durationMin ? ' &middot; ' + last.durationMin + 'min' : ''}</div></>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 8, marginBottom: 16 }}>
          {(ZONE_ORDER as string[]).map(zid => {
            const st = (waterStats as any)[zid]
            return (
              <div key={zid} style={{ background: 'var(--paper)', border: '1px solid var(--border-light,#ede8de)', borderRadius: 4, padding: '8px 12px', fontSize: 12 }}>
                <span style={{ color: 'var(--ink-light)' }}>{(ZONES as any)[zid]?.name}:</span>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>
              {waterEntries.length === 0
                ...
                : waterEntries.map((e: any) => (
                  <tr key={e.id}>
                    <td style={cellStyle}>{e.date}</td>
                    <td style={{ ...cellStyle, fontSize: 12, color: 'var(--ink-mid)' }}>{(e.zones || []).map((z: string) => (ZONES as any)[z]?.name || z).join(', ') || e.zoneNames || '--'}</td>
                    <td style={cellStyle}>{(WATER_METHOD_LABELS as any)[e.method] || e.method || '--'}</td>
                  ...
                  <tr key={e.id}>
                    ...
                    <td style={cellStyle}>
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                        <span style={{ display: 'inline-block', width: 8, height: 8, borderRadius: '50%', background: (TYPE_COLORS as any)[e.taskType] || '#888', marginRight: 6, flexShrink: 0 }} />
                    ...
                    <td style={{ ...cellStyle, fontSize: 12, lineHeight: 1.6 }}>
                      {(e.products || []).length === 0
                        ...
                        : (e.products || []).map((p: any, i: number) => <div key={i}>{p.name} {p.amount}{p.unit}{p.deducted ? ' v' : ''}</div>)
                      }
                    </td>
                    <td style={{ ...cellStyle, fontSize: 12, color: 'var(--ink-mid)' }}>{(e.zones || []).map((z: string) => (ZONES as any)[z]?.name || z).join(', ') || '--'}</td>
                    <td style={cellStyle}>
        ...
        {Object.keys(lastApplied).length === 0
          ...
          : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: 10 }}>
              {Object.entries(lastApplied).sort().map(([name, iso]) => {
                const days = daysSince(iso as string)
                return (
                  <div key={name} style={{ background: 'var(--paper)', border: '1px solid var(--border-light,#ede8de)', borderRadius: 6, padding: '10px 12px' }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--ink-light)' }}>{(iso as string).split('-').reverse().join('/')}</div>
                  </div>
                )
              })}
            </div>
