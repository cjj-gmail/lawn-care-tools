...
export default function DashboardPage() {
  ...
  const [rendered, setRendered] = useState<Set<string>>(() => new Set([paramTab || 'overview']))

  const [stockProduct,     setStockProduct]     = useState<any | null>(null)
  const [resupplyProduct,  setResupplyProduct]  = useState<any | null>(null)
  ...
  const critAlerts = useMemo(() => {
    ...
    return buildAlerts({
      ...
      inventory: state.inventory, invStatusCache: state.invStatusCache,
      completions: state.completions, program: state.program,
    }).filter((a: any) => a.type === 'critical').length
  }, [state])
  ...
  const handleStockSave = useCallback(async (product: any, newQty: number) => {
    const updated = JSON.parse(JSON.stringify(state.inventory))
    const p = updated.products.find((x: any) => x.name === product.name)
  ...
  const handleResupplySave = useCallback(async (product: any, newQty: number) => {
  ...
  const handleManualLogSave = useCallback(async (entry: any) => {
  ...
  const handleWeatherSave = useCallback(async (entry: any) => {
    const entries = (state.weatherLog?.entries || []).filter((e: any) => e.id !== entry.id)
    entries.unshift(entry)
    entries.sort((a: any, b: any) => b.dateISO.localeCompare(a.dateISO))
    const wl = { entries }
  ...
  const handleWeatherDelete = useCallback(async (id: string) => {
  ...
  function openWeatherModal(editId: string | null) {
  ...
  const weatherEditEntry = weatherEditId
    ? (state.weatherLog?.entries || []).find((e: any) => e.id === weatherEditId)
    : null
